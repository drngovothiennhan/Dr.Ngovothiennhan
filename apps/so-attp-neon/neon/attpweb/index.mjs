...
async function proxy(req,u){
  const headers=new Headers(req.headers); headers.delete('host'); headers.delete('origin');
  ...
  const outHeaders=new Headers(r.headers); outHeaders.delete('content-encoding'); outHeaders.delete('content-length');
  return new Response(r.body,{status:r.status,statusText:r.statusText,headers:outHeaders});
}
export default {async fetch(req){
  const u=new URL(req.url);
  if(u.pathname==='/health'||u.pathname.startsWith('/api/')) return proxy(req,u);
